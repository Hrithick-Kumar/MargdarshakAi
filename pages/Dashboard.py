...
# ================= PAGE CONFIG =================
st.set_page_config(page_title="Margdarshak AI Dashboard", page_icon="🎓", layout="wide")
...
st.markdown(
    ...
    <p>Enter your goals, interests and academic performance. Margdarshak AI will recommend the most suitable stream.</p>
    </div>
    """,
    unsafe_allow_html=True
)
...
def load_pref_model():
    ...
    pref_X = pref_data[["Maths_Interest", "Science_Interest", "Business_Interest", "Creativity", "Stress_Handling_Level"]]
    pref_y = pref_data["Stream"]
    pref_model = RandomForestClassifier(n_estimators=100, random_state=42)
    pref_model.fit(pref_X, pref_y)
    return pref_model
pref_model = load_pref_model()

# ================= MARKS MODEL =================
@st.cache_resource
def load_marks_model():
    marks_data = pd.read_csv("StudentMark.csv")
    marks_X = marks_data[["Maths", "Science", "Social Science", "English"]]
    marks_y = marks_data["Stream"]
    marks_model = RandomForestClassifier(n_estimators=100, random_state=42)
    marks_model.fit(marks_X, marks_y)
    return marks_model
marks_model = load_marks_model()

# ================= CAREER MAP =================
career_map = {
    "Science": ["Software Engineer", "Doctor", "Data Scientist", "Research Scientist", "AI Engineer", "NDA Officer"],
    "Commerce": ["CA", "CS", "Investment Banker", "MBA", "Business Analyst"],
    "Arts": ["Lawyer", "Journalist", "UPSC Officer", "Psychologist", "Designer"]
}

# ================= PREDICTION FORM =================
with st.form("prediction_form"):
    st.header("Career Goal Analysis")
    goal_input = st.text_area("Describe your interests, dreams and future goals", height=150)
    ...
    with m2:
        social = st.number_input("Social Science Marks", min_value=0, max_value=100, value=0)
        english = st.number_input("English Marks", min_value=0, max_value=100, value=0)

    predict_btn = st.form_submit_button("🚀 Predict My Stream", use_container_width=True)

# ================= PREDICTION LOGIC =================
if predict_btn:
    if maths == 0 or science == 0 or social == 0 or english == 0:
        st.warning("Please fill all marks.")
    else:
        results = []
        # Goal prediction
        if goal_input.strip():
            goal_vector = vectorizer.transform([goal_input])
            goal_prediction = text_model.predict(goal_vector)[0]
            results.append(goal_prediction)
        # Preference prediction
        pref_input = pd.DataFrame([[maths_interest, science_interest, business_interest, creativity, stress_level]],
                                  columns=["Maths_Interest", "Science_Interest", "Business_Interest", "Creativity", "Stress_Handling_Level"])
        pref_prediction = pref_model.predict(pref_input)[0]
        results.append(pref_prediction)
        # Marks prediction
        marks_prediction = marks_model.predict([[maths, science, social, english]])[0]
...
Your responsibilities: 1. Career Guidance 2. Career Roadmap 3. Future Scope 4. Skill Recommendations 5. Exam Suggestions 6. College Guidance
Rules: • Keep answers simple • Be practical • Use bullet points • Focus on student's stream • Keep answers student friendly
"""},
...
def update_photo():
    ...
    if uploaded:
        ...
        if st.button("Save Photo"):
            os.makedirs("images/updateImg", exist_ok=True)
            file_path = os.path.join("images/updateImg", uploaded.name)
            with open(file_path, "wb") as f:
                f.write(uploaded.getbuffer())
            coll.update_one({"Name": username}, {"$set": {"Photo": f"updateImg/{uploaded.name}"}})
...
def change_password():
    ...
    if st.button("Update Password"):
        user_data = coll.find_one({"Name": username})
        if user_data["Pass"] == old_pass:
            coll.update_one({"Name": username}, {"$set": {"Pass": new_pass}})
...
with st.sidebar:
    ...
    if user:
        ...
        if photo and os.path.exists(image_path):
            st.image(image_path, width=220)
        else:
            st.warning("Profile Image Missing")
    st.divider()
    st.info(f"Recommended Stream\n\n{st.session_state['recommended_stream']}")
    st.divider()
    if st.button("🖼️ Update Photo", use_container_width=True):
        update_photo()
